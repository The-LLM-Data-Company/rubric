"""
Example generate functions for xAI.
...
async def xai_per_criterion_generate_fn(
    system_prompt: str, user_prompt: str, **kwargs: Any
) -> PerCriterionOutput:
    """Generate function for PerCriterionGrader using xAI API with structured outputs."""
    client = AsyncOpenAI(
...
async def xai_oneshot_generate_fn(
    system_prompt: str, user_prompt: str, **kwargs: Any
) -> OneShotOutput:
    """Generate function for PerCriterionOneShotGrader using xAI API with structured outputs."""
    client = AsyncOpenAI(
        api_key=os.getenv("XAI_API_KEY"),
        base_url="https://api.x.ai/v1",
    )
